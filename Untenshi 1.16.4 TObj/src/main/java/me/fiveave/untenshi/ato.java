...
class ato {
    ...
    static void atosys(utsvehicle lv, MinecartGroup mg) {
        ...
        if (lv.getAtodest() != null && lv.getAtospeed() != -1 && lv.getAtsping() == 0 && lv.getAtsforced() == 0 && (lv.getLd() == null || lv.getLd().isAllowatousage())) {
            ...
            */
            ...
            // Cancel braking?
            if (tempdist > reqdist[6] + speed1s(lv) * (getThinkingTime(lv, 6) + 4) || tempdist > reqdist[1] + speed1s(lv) * getThinkingTime(lv, 1)) {
    ...
    static void getAllReqdist(utsvehicle lv, double upperSpeed, double lowerSpeed, double speeddrop, double[] reqdist, double slopeaccel) {
        ...
        for (int a = 1; a <= 8; a++) {
            ...
            reqdist[a] = getReqdist(upperSpeed, lowerSpeed, avgRangeDecel(decel, upperSpeed, lowerSpeed, a + 1, speedsteps), slopeaccel, speeddrop) + upperSpeed / 3.6 * getThinkingTime(lv, a);
        }
    }
